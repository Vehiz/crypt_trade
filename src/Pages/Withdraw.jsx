...
const Withdraw = () => {
  ...
  useEffect(() => {
    ...
    const fetchWithdrawals = async () => {
      if (user.email) {
        setLoading(true);
        const withdrawalCollection = collection(db, "Users", user.email, "Withdrawals");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const docRef = await addDoc(collection(db, "Users", user.email, "Withdrawals"), newWithdrawal);
    ...
    <div className="flex flex-col min-h-screen">
      <main className="flex-grow bg-[#f4f7fe]">
        <section className="flex flex-wrap justify-center">
          <div className="w-full lg:w-1/2 px-4 text-base font-normal mb-8">
            <div className="mt-6 bg-white rounded-lg shadow-md">
              <div className="block p-8">
                ...
                <h3 className="text-2xl mb-[20px] font-semibold">${total.toFixed(2)}</h3>
                <div className="text-gray-700 text-opacity-50">Total Sum</div>
              </div>
            </div>
          </div>

          <div className="w-full mt-7 lg:w-1/2 px-4 text-base font-normal mb-8">
                ...
                <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
                  <div className="flex flex-wrap justify-between">
                    <label className="flex gap-2">
                      <img src={assetDetails.icon} alt={selectedAsset.toLowerCase()} />

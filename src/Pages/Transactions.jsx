...
const Transactions = () => {
  ...
  {deposits.length > 0 ? deposits.map(deposit => (
    <tr key={deposit.id} className="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
      <td className="py-3 px-4 lg:py-4">{deposit.id}</td>
      <td className="py-3 px-4 lg:py-4">{deposit.amount}</td>
      <td className="py-3 px-4 lg:py-4 whitespace-nowrap">{deposit.date}</td>
      <td className="py-3 px-4 lg:py-4">{deposit.method}</td>
      <td className="py-3 px-4 lg:py-4">{deposit.imageUrl !== "" ? "Approved" : "Pending"}</td>
    </tr>
  )) : (
    <tr>
      <td className="text-center px-6 py-6" colSpan="5">No data available in table</td>
    </tr>
  )}
</tbody>

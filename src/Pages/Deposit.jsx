...
const Deposit = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (user) {
      try {
        const imageRef = ref(storage, `deposit-images/${user.email}/${image.name}`);
  ...
  return (
    <section className="bg-[#f4f7fe]">
      <div className="flex flex-wrap justify-between">
        <div className="w-full lg:w-1/2 mx-[15px] lg:mx-[30px] px-[10px]">
        ...
        <div className="w-full lg:w-1/2 bg-[#fff] p-[15px] lg:p-[30px] my-[30px] rounded-lg shadow-outline-white shadow-md border mx-[15px] lg:mx-[30px]">
                ...
                {deposits.length > 0 ? (
                  deposits.map((deposit) => (
                    <tr key={deposit.id}>
                      ...
                      <td className="py-[10px] lg:py-[15px]">{deposit.date}</td>
